...
function fromRow(row: Row): CredentialRow {
  return {
    ...
    applePayDomain: row.apple_pay_domain,
...
export function saveCredentials(
  ...
  const next: CredentialRow = {
    ...
    applePayDomain: patch.applePayDomain ?? current?.applePayDomain ?? null,
  ...
  db.prepare(
    `INSERT INTO credentials (
       ...
       external_key, external_id, apple_pay_domain, merchant_client_id,
       merchant_client_secret, updated_at
     ) VALUES (@environment, @appUuid, @appNumericalId, @appClientId, @appClientSecret,
               @externalKey, @externalId, @applePayDomain, @merchantClientId,
               @merchantClientSecret, @updatedAt)
     ON CONFLICT(environment) DO UPDATE SET
       ...
       apple_pay_domain = excluded.apple_pay_domain,
...
/**
 * Limpa o domínio do Apple Pay de um ambiente, voltando à detecção automática
 * pelo host da requisição.
 *
 * Precisa de função própria porque `saveCredentials` faz upsert parcial, onde
 * ausência significa "mantém o que está lá" — não existe como dizer "apague
 * este campo" por lá. Retorna `false` se não houver banco disponível.
 */
export function clearApplePayDomain(environment: AppmaxEnvironment): boolean {
  const db = getDb();
  if (!db) return false;

  db.prepare(
    `UPDATE credentials SET apple_pay_domain = NULL, updated_at = ? WHERE environment = ?`
 ...
 * merchant), preservando o que é configuração e não muda ao reinstalar: as
 * credenciais do APP, a `external_key` e o domínio do Apple Pay. Retorna
 * `false` se não houver banco disponível.
 */
